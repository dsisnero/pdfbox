  ...
  describe GlyphSubstitutionTable do
    describe "with LiberationSans-Regular.ttf" do
      it "getGsubData() with no args yields latn" do
        ...
        begin
          ...
          gsub_data.active_script_name.should eq("latn")
        ensure
          font.close
        end
      end
      ...
      it "getGsubData() for an unsupported script yields nil" do
        ...
        begin
          ...
          gsub_data = gsub_table.gsub_data("<some_non_existent_script_tag>")
      ...
      it "getGsubData() for 'cyrl' tag yields GSUB features of Cyrillic script" do
        ...
        begin
          ...
          cyrillic_gsub_data = gsub_table.gsub_data(cyrillic_script_tag)
          cyrillic_gsub_data.should_not be_nil
          gsub_data = cyrillic_gsub_data.as(Model::GsubData)
          gsub_data.as(Model::MapBackedGsubData).active_script_name.should eq(cyrillic_script_tag)
          gsub_data.supported_features.should eq(Set.new(expected_features))
        ensure
          font.close
        end
      end
      ...
      it "All the script tags are loaded from GSUB as is" do
        ...
        begin
          ...
          supported_script_tags = gsub_table.supported_script_tags
      ...
      ["DFLT", "bopo", "copt", "cyrl", "grek", "hebr", "latn"].each do |script_tag|
        it "GSUB data is loaded for script #{script_tag}" do
          ...
          begin
            gsub_table = font.table(GlyphSubstitutionTable::TAG).as(GlyphSubstitutionTable)
            gsub_data = gsub_table.gsub_data(script_tag)

            gsub_data.should_not be_nil
            gsub_data.should_not eq(Model::GsubData::NO_DATA_FOUND)
            data = gsub_data.as(Model::GsubData)
            data.language.should eq(Model::Language::UNSPECIFIED)
            data.as(Model::MapBackedGsubData).active_script_name.should eq(script_tag)
          ensure
            font.close
          end
        end
    ...
    it "parses GSUB script list and exposes script-specific gsub data" do
      ...
      begin
        ...
        supported_script_tags = gsub.supported_script_tags
        supported_script_tags.includes?("latn").should be_true
        supported_script_tags.includes?("cyrl").should be_true

        cyrl_data = gsub.gsub_data("cyrl")
        cyrl_data.should_not be_nil
        data = cyrl_data.as(Model::GsubData)
        data.as(Model::MapBackedGsubData).active_script_name.should eq("cyrl")
        data.as(Model::MapBackedGsubData).supported_features.empty?.should be_false
      ensure
        font.close
      end
    end
