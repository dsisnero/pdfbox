module Pdfbox::Pdmodel::Interactive::Annotation::Handlers
  class PDStrikeoutAppearanceHandler < PDAbstractAppearanceHandler
    def generate_normal_appearance : Nil
      annot = wrapped_annotation.as(Annotation::PDAnnotationStrikeout)
      ...
      quad_group_count(paths_array).times do |index|
        x0, y0 = strikeout_start(paths_array, index, annotation_border.width.to_f64)
        x1, y1 = strikeout_end(paths_array, index, annotation_border.width.to_f64)
    ...
    private def strikeout_start(paths_array : Array(Float64), index : Int32, width : Float64) : Tuple(Float64, Float64)
      offset = index * 8
      ...
      {
        paths_array[offset + 4] + ((paths_array[offset] - paths_array[offset + 4]) / len * (len / 2.0_f64 - width)),
        paths_array[offset + 5] + ((paths_array[offset + 1] - paths_array[offset + 5]) / len * (len / 2.0_f64 - width)),
      }
    end

    private def strikeout_end(paths_array : Array(Float64), index : Int32, width : Float64) : Tuple(Float64, Float64)
      offset = index * 8
      ...
      {
        paths_array[offset + 6] + ((paths_array[offset + 2] - paths_array[offset + 6]) / len * (len / 2.0_f64 - width)),
        paths_array[offset + 7] + ((paths_array[offset + 3] - paths_array[offset + 7]) / len * (len / 2.0_f64 - width)),
      }
    end
  end
end
