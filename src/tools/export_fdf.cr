...
module Tools
  class ExportFDF
    ...
    def call(args : Array(String)) : Int32
      ...
      outfile = @outfile || default_output(infile, ".fdf")
      export(infile, outfile)
    rescue ex : ArgumentError
      @err.puts("Error: #{ex.message}")
      1
    rescue ex : IO::Error
      @err.puts("Error exporting FDF data [#{ex.class.name.split("::").last}]: #{ex.message}")
      4
    end
    ...
    private def build_option_parser : OptionParser
      OptionParser.new do |parser|
        ...
        parser.on("-o FILE", "--output FILE", "The FDF data file") { |value| @outfile = value }
      end
    end
