...
module Tools
  class ImportFDF
    ...
    def call(args : Array(String)) : Int32
      ...
      @err.puts("Error importing FDF data [#{ex.class.name.split("::").last}]: #{ex.message}")
      4
    end
    ...
    private def build_option_parser : OptionParser
      OptionParser.new do |parser|
        ...
        parser.on("--data FILE", "The FDF data file to import from") { |value| @data_file = value }
      end
    end
    ...
    private def import(infile : String, outfile : String, data_file : String) : Int32
      ...
      begin
        ...
        form.import_fdf(Pdfbox::Loader.load_fdf(data_file))
